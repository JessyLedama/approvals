...
class sale_order(models.Model):
    ...
    warning_ids = fields.One2many(
        'sale.approval.warnings', 'sale_id', string="Warnings")
    ...
    def check_limit(self):
        ...
        movelines = self.env['account.move.line'].search([
            ...
            '|', ('date_maturity', '=', False), ('date_maturity',
        ...
        currency_balances = [{'balance': sum(line['balance'] for line in lines), 'currency': currency}
                             for currency, lines in lines_per_currency.items()]

        for item in currency_balances:
            if item['balance'] > 0:
                warnings.append((0, 0, {'name': 'Credit Period Violated with Overdue %s : %s' % (
        ...
        return False  # all orders subjected to approval with warnings if necessary
